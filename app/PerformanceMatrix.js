...
import {canonicalNeetNodeId} from "./lib/neetSyllabusMigration";
...
function enrich(q){return q?.exam?.toLowerCase()==="neet"?{...q,node_id:canonicalNeetNodeId({...q,nodeId:q.node_id})}:q;}
function label(q){return q?.concept||q?.topic||q?.subject||q?.node_id||"Unmapped";}
function build(rows){
 const m=new Map();
 for(const raw of rows){const r=enrich(raw);const k=r.node_id||label(r);if(!m.has(k))m.set(k,{key:k,label:label(r),practice:0,practiceCorrect:0,live:0,liveCorrect:0});const x=m.get(k);if(r.source==="practice"){x.practice++;if(r.correct)x.practiceCorrect++;}else{x.live++;if(r.correct)x.liveCorrect++;}}
