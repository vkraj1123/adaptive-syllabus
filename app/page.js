...
export default function Home(){
 ...
 const [user,setUser]=useState(null),[loginId,setLoginId]=useState(""),[loginPw,setLoginPw]=useState(""),[signUp,setSignUp]=useState(false),[signUpName,setSignUpName]=useState("");

 useEffect(()=>{try{const u=JSON.parse(localStorage.getItem("adaptive-user")||"null");if(u){setUser(u);}}catch{}},[]);
 useEffect(()=>{if(!user)return;const p="u:"+user.id+":";try{const sq=JSON.parse(localStorage.getItem(p+"questions")||"[]");const ss=JSON.parse(localStorage.getItem(p+"syllabus")||"[]");setQuestions(sq);
   ...
   setPerformance(JSON.parse(localStorage.getItem(p+"performance")||"{}"));setEvents(JSON.parse(localStorage.getItem(p+"events")||"{}"));setExam(localStorage.getItem(p+"exam")||DEFAULT_EXAM);}catch{}},[user]);
 const uk=user?"u:"+user.id+":":"";
 useEffect(()=>{if(user)localStorage.setItem(uk+"questions",JSON.stringify(questions))},[questions,user]);
 useEffect(()=>{if(user&&syllabus.length)localStorage.setItem(uk+"syllabus",JSON.stringify(syllabus))},[syllabus,user]);
 useEffect(()=>{if(user)localStorage.setItem(uk+"performance",JSON.stringify(performance))},[performance,user]);
 useEffect(()=>{if(user)localStorage.setItem(uk+"events",JSON.stringify(events))},[events,user]);
 useEffect(()=>{if(user)localStorage.setItem(uk+"exam",exam)},[exam,user]);
 ...
 function doLogin(){if(signUp){if(!loginId.trim()||!loginPw.trim()){setMessage("Enter user ID and password.");return;}const users=JSON.parse(localStorage.getItem("adaptive-users")||"{}");if(users[loginId]){setMessage("User ID already exists. Log in instead.");return;}const u={id:loginId.trim(),name:signUpName.trim()||loginId.trim(),password:loginPw};users[loginId]=u;localStorage.setItem("adaptive-users",JSON.stringify(users));localStorage.setItem("adaptive-user",JSON.stringify(u));setUser(u);setLoginId("");setLoginPw("");setSignUpName("");setSignUp(false);setMessage("Account created. Welcome, "+u.name+"!");}else{const users=JSON.parse(localStorage.getItem("adaptive-users")||"{}");const u=users[loginId];if(!u||u.password!==loginPw){setMessage("Invalid credentials.");return;}localStorage.setItem("adaptive-user",JSON.stringify(u));setUser(u);setLoginId("");setLoginPw("");setMessage("Welcome back, "+u.name+"!");}}
 function doLogout(){localStorage.removeItem("adaptive-user");setUser(null);setQuestions([]);setSyllabus([]);setPerformance({});setEvents({});setTest(null);setTab("dashboard");setMessage("Logged out.");}
...
if(!user)return <main style={css.page}><div style={css.wrap} style={{maxWidth:400,marginTop:80}}>
  <div style={{fontSize:13,fontWeight:700,letterSpacing:1}}>ADAPTIVE SYLLABUS</div><h1 style={{margin:"6px 0"}}>{signUp?"Create account":"Sign in"}</h1>
  {message&&<div style={{...css.card,background:"#f0f7ff"}}>{message}</div>}
  {signUp&&<input style={{...css.input,marginBottom:10}} placeholder="Your name" value={signUpName} onChange={e=>setSignUpName(e.target.value)}/>}
  <input style={{...css.input,marginBottom:10}} placeholder="User ID" value={loginId} onChange={e=>setLoginId(e.target.value)}/>
  <input style={{...css.input,marginBottom:14}} type="password" placeholder="Password" value={loginPw} onChange={e=>setLoginPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doLogin()}/>
  <button style={{...css.primary,width:"100%"}} onClick={doLogin}>{signUp?"Create account":"Sign in"}</button>
  <button style={{...css.btn,width:"100%",marginTop:10}} onClick={()=>{setSignUp(!signUp);setMessage("")}}>{signUp?"Already have an account? Sign in":"Need an account? Sign up"}</button>
 </div></main>;
 return <main style={css.page}><div style={css.wrap}>
  <header style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}><div><div style={{fontSize:13,fontWeight:700,letterSpacing:1}}>ADAPTIVE SYLLABUS</div><h1 style={{margin:"6px 0"}}>Your learning system</h1><div style={css.small}>Multi-exam \u2022 concept-level \u2022 deterministic adaptation</div></div><div style={{textAlign:"right"}}><div style={{...css.small,marginBottom:4}}>{user.name}</div><button style={{...css.btn,padding:"5px 10px",fontSize:12}} onClick={doLogout}>Logout</button></div></header>
