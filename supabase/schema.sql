...
-- First cloud signup becomes the initial admin.
-- Run this once in Supabase SQL Editor.

create table if not exists public.profiles (id uuid primary key references auth.users(id) on delete cascade,user_id text unique not null,name text not null,dob date,exam text not null,role text not null default 'student',created_at timestamptz not null default now());
...
-- The first profile created after a clean reset becomes admin.
-- A transaction lock prevents two simultaneous first signups from both becoming admin.
...
begin
  ...
  values(new.id,coalesce(new.raw_user_meta_data->>'user_id',split_part(new.email,'@',1)),coalesce(new.raw_user_meta_data->>'name','Student'),nullif(new.raw_user_meta_data->>'dob','')::date,coalesce(new.raw_user_meta_data->>'exam','ras'),case when first_account then 'admin' else 'student' end)
...
-- ONE-TIME DEVELOPMENT RESET: uncomment these two lines only when you want
-- to delete every cloud account and all related cloud learning data.
